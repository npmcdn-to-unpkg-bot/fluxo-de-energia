...
<script type="text/javascript">
	function getUC(){
		...
		// var c1=products->get(id=product_id) HOW TO GET PRODUCT BY DIFF IDS FROM FRONTEND? 
...
<pre>
	...
	<label>CHOOSE PRODUCT: <select name='product_id' id='product_id'>
		...
		<option value="{{ $prod->id }}">{{ $prod->god->user->username or 'GOD'}}'s'  {{ $prod->category }} {{ $prod->name }} ({{ $prod->id }})</option>
			@endforeach
	</select></label><br>
